...
class ProactiveAppIntallationHelper {
    ...
    async InstallAppInPersonalScope(MicrosoftTenantId, Userid) {
        return new Promise(async (resolve) => {
            const accessToken = await this.GetAccessToken(MicrosoftTenantId);
        ...
        return new Promise(async (resolve) => {
            ...
            axios(config)
                .then(async function(response) {
                    const installedApps = response.data.value.map(element => element.teamsApp.externalId);
                    let result = null;
                    if (installedApps != null) {
                        installedApps.value.forEach(async apps => {
                            result = await objProactiveAppIntallationHelper.InstallAppInPersonalChatScope(accessToken, Userid, apps.id);
                        });
                    }
                    resolve(result);
                })
