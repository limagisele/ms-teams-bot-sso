    ...
    if (taskModuleButtons.length > 0) {
        fetch(`${ window.location.origin }/getAppConfig`)
            ...
            .then((data) => {
                ...
                deepLink.href =
                'https://teams.microsoft.com/l/task/' +
                config.MicrosoftAppID +
                '?url=' +
                ''
                    .concat((0, appRoot)(), '/')
                    .concat(
                        DeelplinkIds.CustomForm +
                        '&height=' +
                        TaskModuleSizes.customform.height +
                        '&width=' +
                        TaskModuleSizes.customform.width +
                        '&title=' +
                        TaskModuleStrings.CustomFormTitle +
                        '&completionBotId=' +
                        config.MicrosoftAppID
                    );
                ...
                for (
                    var _i = 0, taskModuleButtons1 = taskModuleButtons;
                    _i < taskModuleButtons1.length;
                    _i++
                ) {
                    var btn = taskModuleButtons1[_i];
                    btn.addEventListener('click', function() {
                        taskInfo1.url = ''
                            .concat((0, appRoot)(), '/')
                            .concat(this.id.toLowerCase(), '.html');

                        // Define default submitHandler()
                        var submitHandler = function(err, result) {
                            console.log('Err: '.concat(err, '; Result:  + ').concat(result));
                        };
                        switch (this.id.toLowerCase()) {
                        case TaskModuleIds.YouTube:
                            taskInfo1.title = TaskModuleStrings.YouTubeTitle;
                            taskInfo1.size = { height: 7000, width: 1000 };
                            microsoftTeams.dialog.open(taskInfo1, submitHandler);
                            break;

                        case TaskModuleIds.PowerApp:
                            taskInfo1.title = TaskModuleStrings.PowerAppTitle;
                            taskInfo1.size = {
                                height: TaskModuleSizes.powerapp.height,
                                width: TaskModuleSizes.powerapp.width
                            };
                            microsoftTeams.dialog.open(taskInfo1, submitHandler);
                            break;

                        case TaskModuleIds.CustomForm:
                            taskInfo1.title = TaskModuleStrings.CustomFormTitle;
                            taskInfo1.size = {
                                height: TaskModuleSizes.customform.height,
                                width: TaskModuleSizes.customform.width
                            };
                            ...
                            submitHandler = (err, result) => {
                                ...
                                console.log(
                                    `Submit handler - result\rName: ${ result.name }\rEmail: ${ result.email }\rFavorite language: ${ result.favoriteLanguage }`
                                );
                            };

                            // Allows app to open a url based dialog.
                            microsoftTeams.tasks.startTask(taskInfo1, submitHandler);
                            break;

                        case TaskModuleIds.AdaptiveCard1:
                            taskInfo1.title = TaskModuleStrings.AdaptiveCardTitle;
                            taskInfo1.url = '';
                            taskInfo1.size = {
                                height: TaskModuleSizes.powerapp.height,
                                width: TaskModuleSizes.powerapp.width
                            };
                            taskInfo1.card = adaptivecardTemplate;
